...
class RelationFeatureExtractor(nn.Module):
    ...
    def forward(self, x, proposals, rel_pair_idxs=None):
        ...
        for proposal, rel_pair_idx in zip(proposals, rel_pair_idxs):
            ...
            # adjust
            inter_proposal = boxlist_intersection(head_proposal, tail_proposal)
            inter_proposals.append(inter_proposal)
        ...
        inter_vis_features = self.feature_extractor.pooler(x, inter_proposals)
        # merge two parts
        
        if self.separate_spatial:
            region_features = self.feature_extractor.forward_without_pool(union_vis_features)
            inter_region_features = self.feature_extractor.forward_without_pool(inter_vis_features)
            ...
            # union_features = region_features
            inter_features = (inter_region_features, spatial_features)
        else:
            ...
            inter_features = inter_vis_features + rect_features
            inter_features = self.feature_extractor.forward_without_pool(inter_features)
        ...
        if self.cfg.MODEL.ATTRIBUTE_ON:
            ...
            inter_att_features = self.att_feature_extractor.pooler(x, inter_proposals)
            inter_features_att = inter_att_features + rect_features
            inter_features_att = self.att_feature_extractor.forward_without_pool(inter_features_att)
            inter_features = torch.cat((inter_features, inter_features_att), dim=-1)
            # adjust
        
        return union_features, inter_features

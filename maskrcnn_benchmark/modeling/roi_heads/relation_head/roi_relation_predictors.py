...
from .saptial_augmentation import LYContext
...
class MotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        if self.use_bias:
            # convey statistics into FrequencyBias to avoid loading again
            self.freq_bias = FrequencyBias(config, statistics)
        self.spatical_final = LYContext(self.cfg)
...
class CausalAnalysisPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        '''
        import os
        if os.path.exists(self.cfg.OUTPUT_DIR+"errors.json"):
            with open(self.cfg.OUTPUT_DIR+"errors.json", encoding='utf-8') as f:
                line = f.readline()
        '''
        ...
        self.self_study_on = False  # set True if training with self_study
        if self.self_study_on:
            self.errorflag = True
            ...
            self.error_attention = nn.Sequential(*[
                ...
                make_fc(128, 1),
                nn.ReLU(inplace=True)])
            self.adjust_attention = nn.Sequential(*[
                make_fc(151, 1024),
                nn.ReLU(inplace=True),
                nn.BatchNorm1d(1024),
                make_fc(1024, 128),
        ...
        if os.path.exists(self.cfg.OUTPUT_DIR+"error.pt"):
            self.register_buffer("error", torch.load(self.cfg.OUTPUT_DIR+"/error.pt"))
        else:
            self.register_buffer("error", torch.load("/home/ly/Scene-Graph-Benchmark.pytorch/checkpoints/error.pt"))
            torch.save(self.error, self.cfg.OUTPUT_DIR+"/error.pt")
        #if not os.path.exists(self.cfg.OUTPUT_DIR+"error"):
        #    os.makedir(self.cfg.OUTPUT_DIR+"error/")
    ...
    def pair_feature_generate(self, roi_features, proposals, rel_pair_idxs, num_objs, obj_boxs, logger, ctx_average=False):
        ...
        objddd = obj_dists.split(num_objs, dim=0)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        """
        if self.self_study_on:
            if (not self.training) and self.flag:
                torch.save(self.error, self.cfg.OUTPUT_DIR+"/error"+str(self.iter)+".pt")
            else:
                self.iter += 1
        ...
        #print(proposals[0].get_field("predict_logits"))
        #print(proposals[0].get_field("predict_logits").size())
        #assert 1==0
        assert len(num_rels) == len(num_objs)
        ...
        if self.self_study_on:
            ...
            obj_per_image = []
            obj_scene_list = []
            for i in obj_dist_list:
                obj_per_image.append((torch.sum(i, dim=0)/len(i)).unsqueeze(0))
                obj_scene_list.append(torch.zeros(len(i), 2048).to(self.cfg.MODEL.DEVICE))
            obj_per_images = torch.cat(obj_per_image, dim=0)

            obj_scene_feature = self.scene_head(obj_per_images)

            obj_dist_all = torch.max(torch.cat(obj_dist_list, dim=0), 1).indices
            obj_error_list = []

            for i in obj_dist_all:
                obj_error_list.append(self.error[i].unsqueeze(0))

            obj_error_list = torch.cat(obj_error_list, dim=0)
            obj_error_features = self.error_head(obj_error_list) # 2048
            obj_error_attention = self.error_attention(obj_error_list) # 1
            obj_adjust_attention = self.adjust_attention(obj_error_list)


            for i in range(len(obj_scene_list)):
                obj_scene_list[i] += obj_scene_feature[i]
            obj_adjust_feature = torch.cat(obj_scene_list, dim=0)
            obj_adjust = self.adjust_head(obj_adjust_feature * obj_error_attention + obj_error_features)  # adjustment
            #obj_dist_list = (obj_adjust * obj_adjust_attention).split(num_objs, dim=0)
            obj_dist_list = (obj_orignal + obj_adjust * obj_adjust_attention).split(num_objs, dim=0)  # original
        ...
        #     self.obj_dist_save = obj_orignal + obj_adjust * obj_adjust_attention
        #     self.label_save = label_save
        # else:
        #     self.obj_dist_save = torch.cat([self.obj_dist_save, obj_orignal + obj_adjust * obj_adjust_attention], dim=0)      
        ...
        if self.spatial_on:
            all_feature = self.spatical_final(proposals, obj_dist_list, rel_pair_idxs)
            #all_feature = self.spatical_final(proposals, rel_pair_idxs)
        ...
        add_losses = {}
        # add_losses['contra_loss'] = contra_loss
        # additional loss
        if self.training:
        ...
        if self.self_study_on:
            if self.train:
                ...
                h = torch.zeros(151, 151).to(self.cfg.MODEL.DEVICE)
                h[obj_dist_cpu, obj_label_cpu]=1
                self.error = self.error * 0.998 + h

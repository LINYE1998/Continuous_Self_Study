...
from .LYModel_3091 import LYContext
...
class TransformerPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, inter_features, logger=None):
        """
        Returns:
...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, inter_features, logger=None):
        ...
        if self.attribute_on:
            ...
            #print("obj_dist_list: ", obj_dists[0].size())

            #print("[0]: ", obj_dists[0][0])

            #print("rel_dist_list: ", rel_dists[0].size())
            #print("rel_dicts", rel_dists)

            # print("add_loss: ", add_losses)
            return obj_dists, rel_dists, add_losses
...
class CausalAnalysisPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.spatical_on = False #config.MODEL.SPATICAL_ON
        #self.spatical = LYContext(config)#, in_channels)
        ...
        elif config.MODEL.ROI_RELATION_HEAD.CAUSAL.CONTEXT_LAYER == "ly":
            self.context_layer = LYContext(config, obj_classes, rel_classes, in_channels)
        else:
        ...
        self.iter = 0
        self.errorsss = torch.zeros(151, 151)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, inter_features, logger=None):
        ...
        label_save = []
        obj_orignal = torch.cat(obj_dist_list, dim=0)
        for i in proposals:
            label_save.append(i.get_field("labels"))
            label_save = torch.cat(label_save, dim=0)
        if self.iter % 1000 == 0:
            if not self.iter == 0:
                torch.save(self.obj_dist_save, self.cfg.OUTPUT_DIR+"/obj_feature_"+str(self.iter)+".pt")
                torch.save(self.label_save, self.cfg.OUTPUT_DIR+"/label_"+str(self.iter)+".pt")
            self.obj_dist_save = obj_orignal
            self.label_save = label_save
        else:
            self.obj_dist_save = torch.cat([self.obj_dist_save, obj_orignal], dim=0)      
            self.label_save = torch.cat([self.label_save, label_save], dim=0)
        self.iter += 1 
        ...
        if self.spatical_on:
            #all_feature = self.spatical(roi_features, proposals, rel_pair_idxs, rel_labels, logger)
            all_feature = self.spatical(proposals, rel_pair_idxs)
            post_ctx_rep += all_feature
        # print("post", post_ctx_rep.size())
        ...
        '''
        if self.spatical_on:
            obj_dicts, add_rel_dicts = self.spatical(roi_features, proposals, rel_pair_idxs, rel_labels, logger)
            rel_news = ()
            for i in range(len(rel_dist_list)):
                rel_news += (rel_dist_list[i] + add_rel_dicts[i],)
            rel_dist_list = rel_news'''
        # additional loss
        if self.training:
            ...
            rel_dist_list = rel_dists.split(num_rels, dim=0)
        try:
            a = torch.max(torch.cat(obj_dist_list, dim=0), 1).indices
            b = []
            for i in proposals:
                b.append(i.get_field("labels"))
            bs = torch.cat(b, dim=0)
            c = a.to("cpu").numpy().tolist()
            d = bs.to("cpu").numpy().tolist()
            f = torch.zeros(151, 151)
            f[c, d]=1
            self.errorsss += f
        except:
            print("c:", c)
        ...
        torch.save(self.errorsss, self.cfg.OUTPUT_DIR+"/tkh_error_for_paper.pt")
...
# add
@registry.ROI_RELATION_PREDICTOR.register("LYPredictor")
class LYPredictor(nn.Module):
    def __init__(self, config,  obj_classes, rel_classes, statistics, in_channels):
        super(LYPredictor, self).__init__()
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES

        self.context_layer = LYContext(config, in_channels)

    def forward(self, roi_features,  proposals, rel_pair_idxs, rel_labels, rel_binarys, union_features, inter_features, logger=None):
        """
        Returns:
            obj_dists (list[Tensor]): logits of object label distribution
            rel_dists (list[Tensor])
        ...
        obj_dicts, rel_dicts = self.context_layer(roi_features, proposals, rel_pair_idxs, rel_labels, logger)
        add_losses = {}
        return obj_dicts, rel_dicts, add_losses

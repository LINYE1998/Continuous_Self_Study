...
class CausalAnalysisPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        import os
        # print(self.cfg.OUTPUT_DIR+"error.pt")
        if os.path.exists(self.cfg.OUTPUT_DIR+"error.pt"):
            self.error = torch.load(self.cfg.OUTPUT_DIR+"/error.pt")
        else:
            self.error = torch.zeros(151, 151)
            torch.save(self.error, self.cfg.OUTPUT_DIR+"/error.pt")
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        obj_distddd = torch.max(torch.cat(obj_dist_list, dim=0), 1).indices
        obj_labels = []
        for i in proposals:
            obj_labels.append(i.get_field("labels"))
        obj_labelsddd = torch.cat(obj_labels, dim=0)
        obj_dist_cpu = obj_distddd.to("cpu").numpy().tolist()
        obj_label_cpu = obj_labelsddd.to("cpu").numpy().tolist()
        h = torch.zeros(151, 151)
        h[obj_dist_cpu, obj_label_cpu]=1
        self.error += h
        torch.save(self.error, self.cfg.OUTPUT_DIR+"/tkh_error_for_paper.pt")
        ...
        # f[c, d]=1
        # self.error += f
        # torch.save(self.errorsss, self.cfg.OUTPUT_DIR+"/ly_error_for_paper.pt")

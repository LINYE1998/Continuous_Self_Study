...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        #print(len(features))
        ##print("proposals.lengths:", len(proposals))
        #print("proposals:", proposals)
        ##print("proposals0:", proposals[0])
        #print("proposals[0]", proposals[0].bbox)
        #print("proposals0:", proposals[0].fields())
        #print("label:", proposals[0].extra_fields["labels"])
        #print("attributes:", proposals[0].extra_fields["attributes"])
 
        #print("rel_labels:", rel_labels)
        #print("rel_pair_idxs:", rel_pair_idxs)
        #print("rel_binarys:", rel_binarys)
        ...
        if self.use_union_box:
            union_features,  inter_features = self.union_feature_extractor(features, proposals, rel_pair_idxs)
        else:
            union_features = None
            inter_features = None
        ...
        refine_logits, relation_logits, add_losses = self.predictor(proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features,  logger)# inter_features,
        #print("refine_logits: ", refine_logits)
        #print("refine_logits: ", len(refine_logits))
        #print(refine_logits)
        #print("relation_logits: ", relation_logits)
        #print("relation_logits: ", len(relation_logits))
        
        #print("rel_labels:", len(rel_labels))
